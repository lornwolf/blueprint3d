...
var Floorplan = function() {
  ...
  var new_wall_callbacks = JQUERY.Callbacks(); // 暂时没用
  var new_corner_callbacks = JQUERY.Callbacks(); // 暂时没用
  var redraw_callbacks = JQUERY.Callbacks(); // 暂时没用
  var updated_rooms = JQUERY.Callbacks();
  this.roomLoadedCallbacks = JQUERY.Callbacks();

  var defaultTolerance = 10.0;

  // 返回包含所有墙面对象（HalfEdge）的数组
  ...
  // hack
  // 每个二维的Edge对象都包含一个三维的Plane对象（鼠标拾取物体用）
  // 该方法返回所有的Plane对象
  ...
  // 该方法被ThreeFloorplan调用，将回调方法设置为ThreeFloorplan的redraw()方法
  ...
  // 该方法被model.js调用，传入从json数据加载的Floorplan对象
  this.loadFloorplan = function( floorplan ) {
    ...
    // 把参数Floorplan对象的Corner数组加载到本对象中
    for (var id in floorplan.corners) {
      var corner = floorplan.corners[id];
      corners[id] = this.newCorner(corner.x, corner.y, id);
    }
    // 把参数Floorplan对象的Wall数组加载到本对象中
  ...
  this.update = function() {
    ...
    updateFloorTextures();
    // 该方法调用ThreeFloorplan的redraw()方法，重新绘制房间的三维图
    updated_rooms.fire();
  }
  ...
  this.getDimensions = function(center) {
    ...
    var xMin = Infinity; // Infinity : 无穷大
    var xMax = -Infinity;
    var zMin = Infinity;
    var zMax = -Infinity;
  ...
  function assignOrphanEdges() {
    // kinda hacky
    // find orphaned wall segments (i.e. not part of rooms) and give them edges
    orphanWalls = []
    utils.forEach(walls, function(wall) {
